...
/**
 *
 * Patch Apply with Highlight
 */
diff_match_patch.prototype.patch_apply_with_highlight = function(patches, text) {
  if (patches.length == 0) {
    return [text, []];
  }
      ...
      if (text1 == text2) {
        ...
        if (text1.length > this.Match_MaxBits &&
          ...
          for (var y = 0; y < patches[x].diffs.length; y++) {
            ...
            if (mod[0] === DIFF_INSERT) {  // Insertion
              text = text.substring(0, start_loc + index2) + ("[hl]") + mod[1]
                      + ("[ehl]") + text.substring(start_loc + index2);
            } else if (mod[0] === DIFF_DELETE) {  // Deletion
              text = text.substring(0, start_loc + index2) +
                     text.substring(start_loc + this.diff_xIndex(diffs,
                         index1 + mod[1].length));
  ...
  // Strip the padding off.
  if(text.charAt(text.length - (nullPadding.length)) == String.fromCharCode(1))
    text = text.substring(nullPadding.length, text.length - (nullPadding.length));
  else
    text = text.substring(nullPadding.length, text.length - (nullPadding.length + 5)) + "[ehl]";
